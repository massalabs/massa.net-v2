...
export function Navbar() {
  ...
  // Fermer les dropdowns quand on clique ailleurs sur mobile
  useEffect(() => {
    ...
    <div className="uui-text-size-small">Coming soon</div>
        ...
        <div className={`uui-navbar07_menu-button w-nav-button ${open ? 'w--open' : ''}`} onClick={() => setOpen(!open)}>
          <div className="menu-icon_component">
            {open ? (
              <>
                <div className="menu-icon_line-top" style={{ transform: 'rotate(45deg) translate(5px, 5px)' }}></div>
                <div className="menu-icon_line-middle" style={{ opacity: 0 }}></div>
                <div className="menu-icon_line-bottom" style={{ transform: 'rotate(-45deg) translate(7px, -6px)' }}></div>
              </>
            ) : (
              <>
